...
export function addFabric(dataToSubmit, existingFabrics) {
  const request = axios
    .post(`${PRODUCT_SERVER}/fabric`, dataToSubmit)
    .then((response) => {
      let fabrics = [...existingFabrics, response.data.fabric];
      return {
        success: response.data.success,
        fabrics,
      };
